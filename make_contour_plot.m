...
attenuate_pressure = 0;
visualize_conditions = 1;
use_residual_spectra = 0;
use_windowing = 0;
detrend_ensemble = 1; % use esp. if window length is shorter than tidal period
correlations = 1;
...
ea_spacing = 0.5; % What is the spacing between each ensemble?
...
for nn = 0:n_insts-1
    ...
    if detrend_ensemble
        ensemble = detrend(trimmed_depth_signal(si:ei));
    else
        ensemble = trimmed_depth_signal(si:ei);
    end
    
    if use_windowing
        % Uses hamming window, kinda arbitrarily. 
        [W,S] = f_ensemble_average_spectra(T,ensemble.*hamming(length(ensemble)),instance_length*60*60,n_smooth);
    else
...
sni.value = (diffs-min(diffs))./(max(diffs)-min(diffs));
sni.value = movmean(sni.value,8);

% tmp = trimmed_depth_signal - mean(trimmed_depth_signal);
% tmp = tides.data;
% [max_peaks,loc_max] = findpeaks(tmp);
% [min_peaks,loc_min] = findpeaks(-tmp);
% min_peaks = -min_peaks;
% % tmp_times = trimmed_times(floor((loc_max+loc_min)./2));
% tmp_times = tides.time(floor((loc_max(1:end-1)+loc_min)./2));
% % Put timestap halfway between peak and trough, arbitrarily
% [tmp_values,loc_sni] = findpeaks(max_peaks(1:end-1) - min_peaks,'MaxPeakWidth',4);
% sni.time = tmp_times(loc_sni);
% sni.value = tmp_values./max(tmp_values);
% sni.value = movmean(sni.value,6);
...
if correlations
    ...
    % Let local wind waves be 1s to 5s (1 to .2 frequency)
    %   (-0.0000 to -0.6300 in log_10 freq)
    [~,si] = min(abs(freq - 0.2));
    wind_energy = mean(matrixS(:,si:end),2);
    
    % Let swell be 5s to 20s (0.2 to 0.05 frequency)
    %   (-0.6300 to -1.3010 in log_10 freq)
    [~,si] = min(abs(freq - 0.05));
    [~,ei] = min(abs(freq - 0.2));
    swell_energy = mean(matrixS(:,si:ei),2);
    
    % Let igw be 20s to 5min (0.05 to 0.0033333... frequency)
    %   (-1.3010 to -2.4771 in log_10 freq)
    [~,si] = min(abs(freq - 0.00333));
    [~,ei] = min(abs(freq - 0.05));
    igw_energy = mean(matrixS(:,si:ei),2);
    
    % Let seiches be 5min to 30min (0.003333... to 0.00055555... frequency)
    %   (-2.4771 to -3.2553 in log_10 freq)
...
%     figure
%     plot(datenum(ea_times),wind_energy);
%     hold on
%     plot(datenum(ea_times),swell_energy);
%     plot(datenum(ea_times),igw_energy);
%     plot(datenum(ea_times),seiche_energy);
...
%     R = corrcoef(swell_for_corr,swell_energy); R = R(2);
    ...
    figure
    hold on
    plot(ea_times,wind_energy./max(wind_energy));
    plot(ea_times,swell_energy./max(swell_energy));
    plot(ea_times,igw_energy./max(igw_energy));
    plot(ea_times,seiche_energy./max(seiche_energy));
    xlim([ea_times(1) ea_times(end)]);
    title('Normalized Energy from Wind, Swell, IGW, Seiche Bands');
    legend('< 5s','5s to 20s','20s to 5min','5min to 30min');
    
    % Lines to run for autocorrelation
...
contourf(datenum(ea_times(:)),logfreq(2:end),logSt(2:end,:),15,'LineColor','none'); % 9 is pretty good
c = colorbar('east');
if use_residual_spectra
...
% - Automate cmab stuff
% - Adjust conditions visualization to use offshore buoy wind for March-Aug
% 2019, or using marcel's TBB data
